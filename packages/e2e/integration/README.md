...
These scenarios were migrated from `lvce-editor/lvce-editor/scripts/test-simple-browser-*.mjs`. The application-integration workflow runs the tests from this repository against a pinned LVCE source fixture (v0.115.13, including the browser navigation and reload-focus fixes). The fixture supplies the renderer implementation, Electron and build dependencies; it does not supply these tests.
...
To run locally, install a checkout of the pinned application revision, set `LVCE_SOURCE_ROOT` to its absolute directory and run `xvfb-run -a node packages/e2e/integration/test-simple-browser-address.mjs` (substitute another scenario as needed). Run sequentially because source-mode scenarios temporarily bundle the fixture's renderer entry.
