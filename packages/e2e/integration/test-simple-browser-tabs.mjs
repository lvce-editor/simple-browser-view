...
const { getSimpleBrowserVirtualDom } = await import(
  new URL('packages/renderer-worker/src/parts/GetSimpleBrowserVirtualDom/GetSimpleBrowserVirtualDom.js', fixtureUrl)
)
const Elements = await import(new URL('packages/renderer-worker/src/parts/VirtualDomElements/VirtualDomElements.js', fixtureUrl))

const requireTests = createRequire(new URL('packages/extension-host-worker-tests/package.json', fixtureUrl))
const { chromium } = requireTests('playwright')
const css = await readFile(new URL('static/css/parts/ViewletSimpleBrowser.css', fixtureUrl), 'utf8')
